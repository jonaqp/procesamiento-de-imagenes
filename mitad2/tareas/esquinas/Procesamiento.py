...
class Procesamiento:
	...
	def aplicarFiltro(self, abrir, nombreImagenSalida="salidaFILTRO.png"):
		...
		for x in range(self.ancho):
			for y in range(self.alto):
				...
				rec = len(vecindad)/2
				if len(vecindad) % 2 == 0:
					c = (vecindad[rec - 1] + vecindad[rec])/2
				else:
					c = vecindad[rec]
				pixeles[x,y] = (c,c,c)
	...
	def calcularDiferencia(self, abrir, nombreFiltro, nombreImagenSalida="salidaESQUINAS.png"):
		print "Buscando pixeles tipo esquina..."
		pixeles = self.imagen.load()

		filtro = Image.open(nombreFiltro)
		pixelesFiltro = filtro.load()

		for i in range(self.ancho):
			for j in range(self.alto):
				diferencia = pixeles[i,j][1] - pixelesFiltro[i,j][1]
				if diferencia < 0:
					diferencia = 0
				if diferencia > 255:
					diferencia = 255
				...
				if diferencia > 20:	
					pixeles[i,j] = (diferencia,diferencia,diferencia)
				else: # Elimina los puntos muy difusos
					pixeles[i,j] = (0,0,0)	
