...
def analizarBorde(vecindad):
	for tupla in vecindad:
		if 0 in tupla: 
			return False
	return True
...
class TumoresPulmon:
	...
	def engordarBordes(self, abrir, nombreImagenSalida="salidaBORDESGORDOS.png"):
		...
		for x in range(self.ancho):
			for y in range(self. alto):
				vecindad = obtenerVecinos(x, y, self.ancho, self.alto, self.imagenOriginal)
				bordeOrilla = analizarBorde(vecindad)
				# buscamos bordes de las orillas
				if pixeles[x, y] == (255, 255, 255): # buscamos bordes
					if bordeOrilla:
						## engordamos los vecinos de la orilla
						for mx in range(x-2, x+3):
							for my in range(y-2, y+3):
								if mx >= 0 and my >= 0 and mx < self.ancho and my < self.alto:
									# casi blanco para no interferir con la busqueda de blancos
									pixeles[mx,my] = (254, 255, 255) 
			...
			for y in range(self.alto): # ciclo principal
				...
				if negro == pixeles[x,y]:
					...
					if len(masa) > 800 and len(masa) < 2000:
						...
						radioX = puntoCentro[0] - xmin
						radioY = puntoCentro[1] - ymin
						...
						if tumor:
							...
							#draw.ellipse((xmin - radioX, ymin - radioY, xmax + radioX, ymax + radioY), fill=128)
							draw.line((xmin, ymin, xmin, ymax), fill="red")
							draw.line((xmin, ymax, xmax, ymax), fill="red")
							draw.line((xmax, ymax, xmax, ymin), fill="red")
							draw.line((xmax, ymin, xmin, ymin), fill="red")
							#draw.line((xmin-radioX, ymax-radioY, xmax+radioX, ymax+radioY), fill="green")
							#draw.line((xmax+radioX, ymax+radioY, xmax-radioX, ymin+radioY), fill="black")
							#draw.line((xmax-radioX, ymin+radioY, xmin-radioX, ymin-radioY), fill="orange")
						else:
							objeto += 1
